    ...
    describe('#match', function() {
      describe('where the existing (right) order is an offer', function() {
        beforeEach(function() {
          ...
          this.bidBalance = this.account.getBalance('BTC');
          this.offerBalance = this.account.getBalance('EUR');
          this.offerBalance.deposit(amount1000);
          this.order = new Order({
            sequence: 0,
            timestamp: 1371737390976,
            account: this.account,
            bidBalance: this.bidBalance,
            offerBalance: this.offerBalance,
        ...
        describe('and the new (left) price is same', function() {
          describe('and the left order is a bid', function() {
            describe('and the right order is offering exactly the amount the left order is bidding', function() {
              return it.only('should trade the amount the right order is offering, emit a trade event and return false to indicate that no higher trades can be filled by the left order', function() {
                var account, bidBalance, doneSpy, offerBalance, order, tradeSpy,
                  _this = this;
                account = new Account({
                  id: 'Paul',
                  commission: {
                ...
                this.offerBalance.funds.compareTo(Amount.ZERO).should.equal(0);
                this.bidBalance.funds.compareTo(amount199).should.equal(0);
            ...
            describe('and the right order is offering more than the left order is bidding', function() {
              return it('should trade the amount the left order is offering, emit fill events and a trade event and return false to indicate that higher trades may still be filled by the left order', function() {
                ...
                this.fillSpy.firstCall.args[0].timestamp.should.equal('2');
                this.fillSpy.firstCall.args[0].fundsUnlocked.compareTo(amount500).should.equal(0);
                this.order.bidAmount.compareTo(amount100).should.equal(0);
                this.order.offerAmount.compareTo(amount500).should.equal(0);
                tradeSpy.should.not.have.been.called;
                this.tradeSpy.should.have.been.calledOnce;
                this.tradeSpy.firstCall.args[0].timestamp.should.equal(order.timestamp);
                this.tradeSpy.firstCall.args[0].left.sequence.should.equal(order.id);
                this.tradeSpy.firstCall.args[0].left.newBidAmount.compareTo(order.bidAmount).should.equal(0);
                this.tradeSpy.firstCall.args[0].right.sequence.should.equal(this.order.id);
                this.tradeSpy.firstCall.args[0].right.newOfferAmount.compareTo(this.order.offerAmount).should.equal(0);
                this.tradeSpy.firstCall.args[0].price.compareTo(this.order.offerPrice).should.equal(0);
            ...
            return describe('and the right order is offering less than the left order is bidding', function() {
              return it('should trade the amount the right order is offering, emit fill events and a trade event and return true', function() {
                ...
                this.fillSpy.firstCall.args[0].timestamp.should.equal('2');
                this.fillSpy.firstCall.args[0].fundsUnlocked.compareTo(amount1000).should.equal(0);
                this.order.bidAmount.compareTo(Amount.ZERO).should.equal(0);
                this.order.offerAmount.compareTo(Amount.ZERO).should.equal(0);
                tradeSpy.should.not.have.been.called;
                this.tradeSpy.should.have.been.calledOnce;
