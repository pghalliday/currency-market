(function() {
  ...
  newBidOrder = function(bidPrice, sequence) {
    var account, bidBalance, offerBalance;
  ...
  newOfferOrder = function(offerPrice, sequence) {
    var account, bidBalance, offerBalance;
  ...
  describe('Order', function() {
    ...
    it('should throw an error if the bid balance is missing', function() {
      var account;
      ...
      }).to["throw"]('Order must be associated with a bid balance');
    });
    it('should throw an error if the offer balance is missing', function() {
      var account;
      ...
      }).to["throw"]('Order must be associated with an offer balance');
    });
    ...
    it('should error if there are not enough funds available to satisfy the order', function() {
      var account, bidBalance, offerBalance;
    ...
    it('should record the sequence, timestamp, account name, bid balance and offer balance and lock the correct amount of funds in offer balance', function() {
      var account, bidBalance, offerBalance, order;
      ...
      bidBalance = account.getBalance('BTC');
      offerBalance = account.getBalance('EUR');
      offerBalance.deposit(amount5000);
      order = new Order({
        sequence: 0,
        timestamp: 1371737390976,
        account: account,
      ...
      order.bidBalance.should.equal(bidBalance);
      order.offerBalance.should.equal(offerBalance);
      return offerBalance.lockedFunds.compareTo(amount5000).should.equal(0);
    });
    ...
    describe('#match', function() {
      beforeEach(function() {
        ...
        this.bidBalancePeter = this.accountPeter.getBalance('BTC');
        this.offerBalancePeter = this.accountPeter.getBalance('EUR');
        this.offerBalancePeter.deposit(amount1000);
        ...
        this.bidBalancePaul = this.accountPaul.getBalance('EUR');
        this.offerBalancePaul = this.accountPaul.getBalance('BTC');
        return this.offerBalancePaul.deposit(amount375);
      });
      describe('where the existing (right) order is an offer', function() {
        beforeEach(function() {
          this.order = new Order({
            sequence: 0,
            timestamp: 1371737390976,
            account: this.accountPeter,
            bidBalance: this.bidBalancePeter,
            offerBalance: this.offerBalancePeter,
          ...
          describe('and the left order is a bid', function() {
            describe('and the right order is offering exactly the amount the left order is bidding', function() {
              return it('should trade the amount the right order is offering, emit a trade event and return false to indicate that no higher trades can be filled by the left order', function() {
                var doneSpy, order, tradeSpy;

                order = new Order({
                  sequence: 1,
                  timestamp: 1371737390977,
                  account: this.accountPaul,
                  bidBalance: this.bidBalancePaul,
                  offerBalance: this.offerBalancePaul,
                ...
                this.offerBalancePaul.funds.compareTo(amount175).should.equal(0);
                this.bidBalancePaul.funds.compareTo(amount995).should.equal(0);
                this.commissionAccount.getBalance('EUR').funds.compareTo(amount5).should.equal(0);
                this.offerBalancePeter.funds.compareTo(Amount.ZERO).should.equal(0);
                this.bidBalancePeter.funds.compareTo(amount199).should.equal(0);
                this.commissionAccount.getBalance('BTC').funds.compareTo(amount1).should.equal(0);
                ...
                this.tradeSpy.firstCall.args[0].timestamp.should.equal(order.timestamp);
                this.tradeSpy.firstCall.args[0].left.sequence.should.equal(order.sequence);
                this.tradeSpy.firstCall.args[0].left.newBidAmount.compareTo(order.bidAmount).should.equal(0);
                this.tradeSpy.firstCall.args[0].left.balanceDeltas.debit.amount.compareTo(amount200).should.equal(0);
                this.tradeSpy.firstCall.args[0].left.balanceDeltas.credit.amount.compareTo(amount995).should.equal(0);
                this.tradeSpy.firstCall.args[0].left.balanceDeltas.credit.commission.amount.compareTo(amount5).should.equal(0);
                this.tradeSpy.firstCall.args[0].left.balanceDeltas.credit.commission.reference.should.equal('Paul commission level');
                this.tradeSpy.firstCall.args[0].right.sequence.should.equal(this.order.sequence);
                this.tradeSpy.firstCall.args[0].right.newOfferAmount.compareTo(this.order.offerAmount).should.equal(0);
                this.tradeSpy.firstCall.args[0].right.balanceDeltas.debit.amount.compareTo(amount1000).should.equal(0);
                this.tradeSpy.firstCall.args[0].right.balanceDeltas.credit.amount.compareTo(amount199).should.equal(0);
                this.tradeSpy.firstCall.args[0].right.balanceDeltas.credit.commission.amount.compareTo(amount1).should.equal(0);
                this.tradeSpy.firstCall.args[0].right.balanceDeltas.credit.commission.reference.should.equal('Peter commission level');
                this.tradeSpy.firstCall.args[0].price.compareTo(this.order.offerPrice).should.equal(0);
            ...
            describe('and the right order is offering more than the left order is bidding', function() {
              return it('should trade the amount the left order is offering, emit fill events and a trade event and return false to indicate that higher trades may still be filled by the left order', function() {
                ...
                this.offerBalancePaul.funds.compareTo(amount275).should.equal(0);
                this.bidBalancePaul.funds.compareTo(amount495).should.equal(0);
                this.commissionAccount.getBalance('EUR').funds.compareTo(amount5).should.equal(0);
                this.offerBalancePeter.funds.compareTo(amount500).should.equal(0);
                this.bidBalancePeter.funds.compareTo(amount99).should.equal(0);
                this.commissionAccount.getBalance('BTC').funds.compareTo(amount1).should.equal(0);
                order.offerAmount.compareTo(Amount.ZERO).should.equal(0);
                order.bidAmount.compareTo(Amount.ZERO).should.equal(0);
                ...
                this.tradeSpy.firstCall.args[0].left.sequence.should.equal(order.sequence);
                this.tradeSpy.firstCall.args[0].left.newBidAmount.compareTo(order.bidAmount).should.equal(0);
                this.tradeSpy.firstCall.args[0].left.balanceDeltas.debit.amount.compareTo(amount100).should.equal(0);
                this.tradeSpy.firstCall.args[0].left.balanceDeltas.credit.amount.compareTo(amount495).should.equal(0);
                this.tradeSpy.firstCall.args[0].left.balanceDeltas.credit.commission.amount.compareTo(amount5).should.equal(0);
                this.tradeSpy.firstCall.args[0].left.balanceDeltas.credit.commission.reference.should.equal('Paul commission level');
                this.tradeSpy.firstCall.args[0].right.sequence.should.equal(this.order.sequence);
                this.tradeSpy.firstCall.args[0].right.newOfferAmount.compareTo(this.order.offerAmount).should.equal(0);
                this.tradeSpy.firstCall.args[0].right.balanceDeltas.debit.amount.compareTo(amount500).should.equal(0);
                this.tradeSpy.firstCall.args[0].right.balanceDeltas.credit.amount.compareTo(amount99).should.equal(0);
                this.tradeSpy.firstCall.args[0].right.balanceDeltas.credit.commission.amount.compareTo(amount1).should.equal(0);
                this.tradeSpy.firstCall.args[0].right.balanceDeltas.credit.commission.reference.should.equal('Peter commission level');
                this.tradeSpy.firstCall.args[0].price.compareTo(this.order.offerPrice).should.equal(0);
          ...
          return describe('and the left order is an offer', function() {
            describe('and the right order is offering exactly the amount the left order is offering', function() {
              return it('should trade the amount the right order is offering, emit a fill events and a trade event and return false', function() {
                ...
                this.tradeSpy.firstCall.args[0].timestamp.should.equal(order.timestamp);
                this.tradeSpy.firstCall.args[0].left.sequence.should.equal(order.sequence);
                this.tradeSpy.firstCall.args[0].left.newOfferAmount.compareTo(order.offerAmount).should.equal(0);
                this.tradeSpy.firstCall.args[0].left.balanceDeltas.debit.amount.compareTo(amount200).should.equal(0);
                this.tradeSpy.firstCall.args[0].left.balanceDeltas.credit.amount.compareTo(amount995).should.equal(0);
                this.tradeSpy.firstCall.args[0].left.balanceDeltas.credit.commission.amount.compareTo(amount5).should.equal(0);
                this.tradeSpy.firstCall.args[0].left.balanceDeltas.credit.commission.reference.should.equal('Paul commission level');
          ...
          describe('and the left order is an offer', function() {
            describe('and the right order is bidding exactly the amount that the left order is offering', function() {
              return it('should trade the amount the right order is bidding at the right order price, emit fill events and a trade event and return false', function() {
                ...
                this.tradeSpy.firstCall.args[0].left.balanceDeltas.credit.commission.reference.should.equal('Paul commission level');
                this.tradeSpy.firstCall.args[0].right.sequence.should.equal(this.order.sequence);
                this.tradeSpy.firstCall.args[0].right.newBidAmount.compareTo(this.order.bidAmount).should.equal(0);
                this.tradeSpy.firstCall.args[0].right.balanceDeltas.debit.amount.compareTo(amount1000).should.equal(0);
                this.tradeSpy.firstCall.args[0].right.balanceDeltas.credit.amount.compareTo(amount199).should.equal(0);
                this.tradeSpy.firstCall.args[0].right.balanceDeltas.credit.commission.amount.compareTo(amount1).should.equal(0);
                this.tradeSpy.firstCall.args[0].right.balanceDeltas.credit.commission.reference.should.equal('Peter commission level');
                this.tradeSpy.firstCall.args[0].price.compareTo(this.order.bidPrice).should.equal(0);
    ...
    describe('#export', function() {
      return it('should return a JSON stringifiable object containing a snapshot of the order', function() {
        var account, balanceBTC, balanceEUR, json, object, order;
