  ...
  describe('State', function() {
    ...
    return describe('#apply', function() {
      ...
      describe('submit delta', function() {
        return it('should update the state accordingly', function() {
          var balance, bookBTCEUR, bookEURBTC, order, state;
          state = state = new State({
            json: JSON.stringify(this.engine),
            commission: {
              account: 'commission'
          ...
          return bookBTCEUR[0].should.equal(order);
        });
      });
      return describe('cancel delta', function() {
        return it('should update the state accordingly', function() {
          var balance, bookBTCEUR, bookEURBTC, state;
          state = state = new State({
            json: JSON.stringify(this.engine),
            commission: {
              account: 'commission'
          ...
          return bookEURBTC[0].should.equal(state.getAccount('Paul').orders[7]);
        });
      });
    });
  });
