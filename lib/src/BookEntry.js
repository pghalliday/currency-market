  ...
  module.exports = BookEntry = (function() {
    function BookEntry(params) {
      ...
      this["export"] = __bind(this["export"], this);      if (params.state) {
        this.order = params.orders[params.state.order];
    ...
    BookEntry.prototype.add = function(bookEntry) {
      ...
      if (this.order.bidPrice) {
        if (bookEntry.order.bidPrice) {
          isHigher = bookEntry.order.bidPrice.compareTo(this.order.bidPrice) > 0;
        } else {
          isHigher = bookEntry.order.offerPrice.multiply(this.order.bidPrice).compareTo(Amount.ONE) < 0;
        }
      } else {
        if (bookEntry.order.offerPrice) {
          isHigher = bookEntry.order.offerPrice.compareTo(this.order.offerPrice) < 0;
        } else {
          isHigher = bookEntry.order.bidPrice.multiply(this.order.offerPrice).compareTo(Amount.ONE) > 0;
        }
      }
      if (isHigher) {
        if (this.higher) {
