(function() {
  ...
  module.exports = Order = (function() {
      ...
      this["export"] = __bind(this["export"], this);      if (typeof params.state === 'undefined') {
        ...
        if (typeof params.offerPrice === 'undefined') {
          if (typeof params.bidPrice === 'undefined') {
            ...
            this.fillOffer = false;
            this.bidPrice = new Amount(params.bidPrice);
            if (this.bidPrice.compareTo(Amount.ZERO) < 0) {
              ...
              if (typeof params.bidAmount === 'undefined') {
                ...
                this.bidAmount = new Amount(params.bidAmount);
          ...
          this.fillOffer = true;
          this.offerPrice = new Amount(params.offerPrice);
          if (this.offerPrice.compareTo(Amount.ZERO) < 0) {
            ...
            if (typeof params.offerAmount === 'undefined') {
              ...
              this.offerAmount = new Amount(params.offerAmount);
        ...
        this.bidPrice = new Amount(params.state.bidPrice);
        this.bidAmount = new Amount(params.state.bidAmount);
        this.offerPrice = new Amount(params.state.offerPrice);
        this.offerAmount = new Amount(params.state.offerAmount);
        this.fillOffer = params.state.fillOffer;
      }
    }
    ...
    Order.prototype["export"] = function() {
      ...
      state.bidPrice = this.bidPrice.toString();
      state.bidAmount = this.bidAmount.toString();
      state.offerPrice = this.offerPrice.toString();
      state.offerAmount = this.offerAmount.toString();
      state.fillOffer = this.fillOffer;
      return state;
    };

    Order.prototype.equals = function(order) {
      return this.id === order.id && this.timestamp === order.timestamp && this.account === order.account && this.bidCurrency === order.bidCurrency && this.offerCurrency === order.offerCurrency && this.bidPrice.compareTo(order.bidPrice) === 0 && this.bidAmount.compareTo(order.bidAmount) === 0 && this.offerPrice.compareTo(order.offerPrice) === 0 && this.offerAmount.compareTo(order.offerAmount) === 0 && this.fillOffer === order.fillOffer;
    };
