  ...
  module.exports = Account = (function() {
    function Account(params) {
      ...
      if (typeof params.state === 'undefined') {
        ...
        Object.keys(params.state.orders).forEach(function(orderId) {
          return _this.orders[orderId] = new Order({
    ...
    Account.prototype["export"] = function() {
      ...
      Object.keys(this.orders).forEach(function(orderId) {
        return state.orders[orderId] = _this.orders[orderId]["export"]();
      });
      return state;
    };
    ...
    Account.prototype.submit = function(order) {
      ...
      return order.on('fill', function(fill) {
        if (order.bidAmount.compareTo(Amount.ZERO) === 0) {
          delete _this.orders[order.id];
        }
        if (order.offerPrice) {
          return _this.balances[order.offerCurrency].unlock(fill.offerAmount);
        } else {
          return _this.balances[order.offerCurrency].unlock(fill.bidAmount.multiply(order.bidPrice));
        }
      });
    };
